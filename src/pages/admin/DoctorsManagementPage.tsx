...
const EditDoctorModal = lazy(() =>
  import("../../features/admin/EditDoctorModal").then((m) => ({
    default: m.EditDoctorModal,
  })),
);
const AddDoctorModal = lazy(() =>
  import("../../features/admin/AddDoctorModal").then((m) => ({
    default: m.AddDoctorModal,
  })),
);
    ...
    <div className="p-4 md:p-8">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
        <h1 className="text-xl md:text-2xl font-bold text-primary">
          {td("dashboard.admin.doctorsManagement")}
        </h1>
      ...
      <div className="flex flex-col md:flex-row gap-4 w-full mb-6">
        <SearchInput
          ...
          onChange={searchProps.setSearchTerm}
        />
        <DoctorFilters
                ...
                {doctors.map((doc) => (
                  <tr key={doc.id} className="shadow-sm">
                    ...
                    <td className="p-4">
                      {doc.is_active ? (
                        <span className="text-green-500 flex items-center gap-1">
                          <LuUserRoundCheck /> Active
                        </span>
                      ) : (
                        <span className="text-red-500 flex items-center gap-1">
                          <LuUserRoundX /> Inactive
                        </span>
                      )}
                    </td>
                    <td className="p-4 text-start">
                      <button
                        onClick={() =>
                          actions.toggleVerify(doc.id, !!doc.is_verified)
                        }
                      >
                        <LuBadgeCheck
                          className={
                    ...
                    <td className="p-4 text-start">
                      <div className="flex justify-center gap-3 ">
                        <button
                          ...
                          className="bg-tertiary-100 p-2 rounded-[4px] text-white "
                        >
                          <LuPencil size={14} />
                        </button>
                        {selectedDoctor && (
                        ...
                        <button
                          onClick={() =>
                            actions.toggleStatus(doc.id, !!doc.is_active)
                          }
                          className={
                            doc.is_active
